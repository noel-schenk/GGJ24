...
export const playTTS = (message: string, voice = "Google Nederlands") => {
  if ("speechSynthesis" in window) {
    ...
    const voiceInstance = voices.find((ssvoice) => ssvoice.name === voice) ||
...
const splitMessage = (message: string, voice: SpeechSynthesisVoice, pitch: number = 1, rate: number = 1) => {
  const parts = message/*.replaceAll(/[^\w\.,;]/g, ' ')*/.split(' ');
